...
async function main() {
	try {
		...
		if (createRelease) { // Build, pack and release
			...
			await exec(`"C:\\Program Files (x86)\\Microsoft Visual Studio\\2019\\Enterprise\\MSBuild\\Current\\Bin\\MSBuild.exe" ${solutionFile} ${configurationFlag} ${enforceAddingFilesFlag}`);
